...
export const StatsChart = ({ data, color = '#22d3ee', unit = '', label = '' }: StatsChartProps) => {
  // Use a stable gradient ID based on color to avoid conflicts
      ...
      {label && (
        <div className="flex justify-between items-center mb-1 px-1">
           ...
           <span className="text-sm font-bold text-slate-200 font-mono">
             {data.length > 0 ? `${data[data.length - 1].value.toFixed(2)}${unit ? ' ' + unit : ''}` : '-'}
           </span>
        </div>
      )}
      <ResponsiveContainer width="100%" height={label ? '85%' : '100%'}>
        <AreaChart data={data}>
          <defs>
